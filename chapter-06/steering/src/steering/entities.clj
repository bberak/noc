(ns steering.entities
  (:require [quil.core :as q]
            [steering.helpers :refer :all]
            [steering.ces :as ces]
...
(defn particle [physics pos]
  (let [verlet-particle (VerletParticle2D. pos)
        repelling-behavior (AttractionBehavior. verlet-particle 80 -1)]
...
(defn attractor-particle [physics pos]
  (let [verlet-particle (VerletParticle2D. pos)
        attracting-behavior (AttractionBehavior. verlet-particle 800 0.1)
        repelling-behavior (AttractionBehavior. verlet-particle 160 -1)]
    (.addParticle physics verlet-particle)
    (.addBehavior physics attracting-behavior)
    (.addBehavior physics repelling-behavior)
    (ces/entity {:particle verlet-particle
                 :renderable r/attractor-particle})))
...
(defn pendulum [physics pos]
  (let [particles (map (fn [x]
                         (let [p (VerletParticle2D. (.add pos (Vec2D. x 0)))
                               attracting-behavior (AttractionBehavior. p 600 0.8)
                               repelling-behavior (AttractionBehavior. p 200 -1)]
...
(defn graph [physics center]
  (let [nodes (map (fn [i]
                     (let [pos (.add center (.scale (Vec2D/randomVector) (Vec2D. 20 20)))
        ...
        dist-func (fn [mouse-pos p]
                    (let [particle-pos (Vec2D. (.x p) (.y p))] 
                      (.distanceTo mouse-pos particle-pos)))
        find-target (fn [pos components]
                      (let [pos-vec (Vec2D. (first pos) (second pos))
                            nodes (:nodes components)]
                        (or (get-in components [:mouse-input :target]) (first (filter #(< (dist-func pos-vec %) hit-radius) nodes)))
                        ))]
    ...
    (ces/entity {:graph nil
                 :nodes nodes
                 :renderable r/graph
                 :mouse-input {:on-mouse-drag (fn [mouse-button start-pos end-pos components]
                                                (let [target (find-target start-pos components)]
                                                  (if (not-nil? target)
                                                    (let []
                                                      (.lock target)
                                                      ...
                                                      (.unlock target)
                                                      (assoc-in components [:mouse-input :target] target))
                                                    components)))
                             :on-mouse-up (fn [mouse-button pos components]
                                            (let [target (find-target pos components)]
                                              (if (not-nil? target)
                                                (assoc-in components [:mouse-input :target] nil)
                             ...
                             :on-mouse-down (fn [mouse-button pos components]
                                                (let [target (find-target pos components)]
                                                  (if (not-nil? target)
                                                    (let []
                                                      (.lock target)
                                                      (set! (. target x) (+ (first pos)))
                                                      (set! (. target y) (+ (second pos)))                                              
                                                      (.unlock target)
                                                      (assoc-in components [:mouse-input :target] target))
                                                    components)))
